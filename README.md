...
以下の 4 サービス + 1 バッチを最小構成で実装済みです。

- `batch_ingestion`: 手動実行のバッチ取り込み（常駐しない）
- `embedding`: `POST /embed` を提供
- `retrieval`: `POST /search` を提供
- `llm_generation`: `POST /generate` を提供
- `frontend`: React + TypeScript の質問 UI

現在のスケルトンでは、外部依存が未起動でも動作確認できるように以下を採用しています。

- Embedding Service は軽量モックで sparse vector map を返却
- Retrieval Service は Elasticsearch 8 系 `sparse vector` 検索を実行（依存障害時はフォールバック応答）
...
# Batch Ingestion を必要時だけ1回実行
make run-batch-ingestion-docker

# project を指定して直接実行する場合
docker compose run --rm --profile manual batch-ingestion-service \
  python -m batch_ingestion.app.main --project <cosense_project> --limit-pages 50
```

`docker-compose.yml` は各サービス専用 Dockerfile を使って個別イメージをビルドします。

- `embedding/Dockerfile`
- `batch_ingestion/Dockerfile`
- `retrieval/Dockerfile`
- `llm_generation/Dockerfile`
- `frontend/Dockerfile`

初回起動または Dockerfile 変更後に明示的に再ビルドしたい場合:
...
- `make run-batch-ingestion`: Batch Ingestion を1回実行（手動トリガー）
- `make run-batch-ingestion-docker`: Batch Ingestion を docker compose で1回実行
...
- `make run-frontend`: Frontend をローカル起動（`5173`）
...
- `make check`: 各コンポーネント配下の Python 構文チェックを実行
...
- `batch_ingestion` は API ではなく CLI バッチです
  - 実行: `python -m batch_ingestion.app.main --project <cosense_project> --limit-pages 50`
  - 出力: `{ "success_count": 10, "failure_count": 0, "processed_pages": 3, "processed_chunks": 10, "duration_ms": 1234 }`
...
- `Frontend`（React + TypeScript）
  ...
  - 振る舞い: `Idle/Loading/Success/Error` を明示し、`citation` と `再試行` を表示
